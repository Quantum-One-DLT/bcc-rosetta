...
const buildOperation = (
  ...
  const bccBalance = BigInt(balances.balances[0].value);
  let outputAmount = (bccBalance * BigInt(outputsPerc)) / BigInt(100);
  if (isRegisteringStakeKey) {
    let i = 0;
    do {
      let dividend = outputsPerc - i;
      outputAmount = (bccBalance * BigInt(dividend)) / BigInt(100);
      i += 5;
      if (outputAmount < 2500000)
        throw new Error(
          `outputAmount=${outputAmount} is too low. Try with more funds.`
        );
    } while (bccBalance - outputAmount <= 2000000);
  }
