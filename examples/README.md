...
1. Launch `bcc-rosetta` server at port `8080`.
...
It generates an address based on a predefined private key, asks for funds and once found the address is drained and sent to a specific address. This example resembles how a Bcc based payment processing system could be implemented:
...
- You can request for testnet funds using the following [faucet](https://testnets.bcc.org/en/bcc/tools/faucet/).
...
[waitForFunds] Funds found! 81000000 BCC
...
## Sending transactions with bcc-address example
This example uses extended keys that have been created using [`bcc-address`](https://github.com/The-Blockchain-Company/bcc-addresses) with the following commands:
```bash
...
bcc-address key from-recovery-phrase Sophie < seed.txt > root.xsk
...
In order to execute the example, run:
```javascript
$ yarn send-transaction-using-bcc-address-example
```

## Sending transactions with Cole input

Uses a predefined Cole address to send BCC to `SEND_FUNDS_ADDRESS` address.

Important notes:
  - Cole address is generated outside Rosetta.

In order to execute the example, run:

```javascript
$ yarn send-transaction-with-cole-example
```
...
3. All the BCC is also sent to that address 

Important notes:
- `EXPECTED_TOKEN` will be used to define the token expected. It requires policy and symbol both defined as hex string.
- Running this example requieres a sender that owns tokens (for example by minting) and can send them to the rosetta-cli generated address. It's strongly recommended to use [this helper scripts](https://github.com/james-tbco/scripts)
- Funds will be sent to `SEND_FUNDS_ADDRESS` by default as it's the address where testnet funds should be returned.
...
1. All the BCC is sent to a predefined address `SEND_FUNDS_ADDRESS`
...
- A minimum of 502 BCC + fees balance is needed in order to pay for pool registration deposit and stake key registration deposit.
- Funds will be sent to `SEND_FUNDS_ADDRESS` by default as it's the address where testnet funds should be returned.
