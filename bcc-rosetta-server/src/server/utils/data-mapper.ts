...
import { AuxiliaryData } from 'bcc-serialization-lib';
...
/**
 * Converts a Bcc Transaction into a Rosetta one
 *
...
/**
 * Returns a Rosetta block based on a Bcc block and it's transactions
 *
 * @param block bcc block
 * @param transactions bcc transactions for the given block
 */
...
export const mapToAccountCoinsResponse = (blockCoinsData: BlockUtxos): Components.Schemas.AccountCoinsResponse => {
  const mappedUtxos = blockCoinsData.utxos.reduce((bccCoins, current, index) => {
  ...
  if (isBlockUtxos(blockBalanceData)) {
    const maBalances =
      blockBalanceData.maBalances.length > 0
        ...
        : [];
    const bccBalance = blockBalanceData.utxos.reduce((totalAmount, current, index) => {
      const previousValue = blockBalanceData.utxos[index - 1];
      // This function accumulates BCC value. As there might be several, one for each multi-asset, we need to
    ...
    const totalBalance = [mapAmount(bccBalance.toString()), ...maBalances];
    return {
      block_identifier: {
...
/**
 * Returns the BccNetoworkIdentifier based on the Rosetta API one
 *
...
export const getNetworkIdentifierByRequestParameters = (
  ...
  if (networkRequestParameters.network === MAINNET) {
    return NetworkIdentifier.BCC_MAINNET_NETWORK;
  }
  return NetworkIdentifier.BCC_TESTNET_NETWORK;
};
 ...
 *
 * CBOR is being used to follow standard Bcc serialization library
 *
 * @param transaction
 * @param extraData
 */
...
// if BCC is requested then all coins will be returned
export const filterRequestedCurrencies = (currencies?: Components.Schemas.Currency[]): Components.Schemas.Currency[] =>
  currencies && !currencies.map(currency => currency.symbol).some(s => s === BCC) ? currencies : [];
