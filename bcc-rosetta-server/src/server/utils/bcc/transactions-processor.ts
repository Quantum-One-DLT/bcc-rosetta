import BccWasm, { AuxiliaryData, BigNum } from 'bcc-serialization-lib';
...
const parseInputToOperation = (input: BccWasm.TransactionInput, index: number): Components.Schemas.Operation => ({
...
const parsePoolMetadata = (poolParameters: BccWasm.PoolParams): Components.Schemas.PoolMetadata | undefined => {
...
export const parsePoolOwners = (
  logger: Logger,
  network: number,
  poolParameters: BccWasm.PoolParams
): Array<string> => {
  ...
  for (let i = 0; i < ownersCount; i++) {
    ...
    const address = generateRewardAddress(logger, network, BccWasm.StakeCredential.from_keyhash(owner));
...
const parseIpv4 = (wasmIp?: BccWasm.Ipv4): string | undefined => {
  if (wasmIp) {
...
const parseIpv6 = (wasmIp?: BccWasm.Ipv6): string | undefined => {
  if (wasmIp) {
...
const parsePoolRelays = (poolParameters: BccWasm.PoolParams): Array<Components.Schemas.Relay> => {
...
const parsePoolMargin = (poolParameters: BccWasm.PoolParams): Components.Schemas.PoolMargin => ({
...
const parsePoolRegistration = (
  logger: Logger,
  network: number,
  poolRegistration: BccWasm.PoolRegistration
 ...
 * @param  {BccWasm.Assets|BccWasm.MultiAsset} collection
 * @returns BccWasm
 */
const keys = (
  collection: BccWasm.Assets | BccWasm.MultiAsset
): (BccWasm.AssetName | BccWasm.ScriptHash)[] => {
  const keysArray = [];
...
const parseAsset = (
  logger: Logger,
  assets: BccWasm.Assets,
  key: BccWasm.AssetName
...
const parseTokenAsset = (
  logger: Logger,
  multiassets: BccWasm.MultiAsset,
  multiassetKey: BccWasm.ScriptHash
  ...
  return {
    policyId,
    tokens: (keys(assets) as BccWasm.AssetName[])
...
const parseTokenBundle = (
  logger: Logger,
  output: BccWasm.TransactionOutput
  ...
  if (multiassets) {
    ...
    tokenBundle = (keys(multiassets) as BccWasm.ScriptHash[])
...
const parseOutputToOperation = (
  ...
  amount: {
    value: output
      ...
      .coin()
      .to_str(),
    currency: { symbol: BCC, decimals: BCC_DECIMALS }
  },
  status: '',
...
const parseCertsToOperations = (
  logger: Logger,
  transactionBody: BccWasm.TransactionBody,
...
const parseVoteMetadataToOperation = (
  ...
  const generalMetadata = BccWasm.GeneralTransactionMetadata.from_bytes(
    BccWasm.MetadataList.from_bytes(transactionMetadata.to_bytes())
      .get(0)
      .to_bytes()
  );
  ...
  const dataJson = BccWasm.decode_metadatum_to_json_str(data, BccWasm.MetadataJsonSchema.BasicConversions);
  const sigJson = BccWasm.decode_metadatum_to_json_str(sig, BccWasm.MetadataJsonSchema.BasicConversions);
...
/**
 * Converts a Bcc Transaction into a Rosetta Operation array.
 *
 * @param logger
