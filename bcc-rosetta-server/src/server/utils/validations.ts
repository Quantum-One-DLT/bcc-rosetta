import { PUBLIC_KEY_BYTES_LENGTH, BCC, AddressType, CurveType, ASSET_NAME_LENGTH, POLICY_ID_LENGTH } from './constants';
import { ErrorFactory } from './errors';
import { isEmptyHexString } from './formatters';
import BccWasm from 'bcc-serialization-lib';
...
export const validateCurrencies = (currencies: Components.Schemas.Currency[]): void => {
  currencies.forEach(({ symbol, metadata }) => {
    ...
    if (symbol !== BCC && !isPolicyIdValid(metadata.policyId))
...
export const isEd25519KeyHash = (hash: string): boolean => {
  let edd25519Hash: BccWasm.Ed25519KeyHash;
  try {
    edd25519Hash = BccWasm.Ed25519KeyHash.from_bytes(Buffer.from(hash, 'hex'));
  } catch (error) {
    return false;
  }
  return !!edd25519Hash;
