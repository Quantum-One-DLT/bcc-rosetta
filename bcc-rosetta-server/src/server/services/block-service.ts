...
import { BccService, LinearFeeParameters } from './bcc-services';
...
const configure = (repository: BlockchainRepository, bccService: BccService): BlockService => ({
  async findBlock(logger, number, hash) {
    logger.info({ number, hash }, '[findBlock] Looking for block:');
    // bcc doesn't have block zero but we need to map it to genesis
  ...
  async findBalanceDataByAddressAndBlock(logger, address, number, hash) {
    ...
    if (bccService.isStakeAddress(address)) {
