...
import BccWasm, { BigNum, Ed25519Signature, PublicKey, StakeCredential, Vkey } from 'bcc-serialization-lib';
...
import {
  ...
  getEraAddressType
} from '../utils/bcc/addresses';
import * as OperationsProcessor from '../utils/bcc/operations-processor';
import { getStakingCredentialFromHex } from '../utils/bcc/staking-credentials';
import * as TransactionProcessor from '../utils/bcc/transactions-processor';
import {
  AddressType,
  ATTRIBUTES_LENGTH,
  CHAIN_CODE_LENGTH,
...
// Sophie
const SOPHIE_DUMMY_SIGNATURE = new Array(SIGNATURE_LENGTH + 1).join('0');
const SOPHIE_DUMMY_PUBKEY = new Array(PUBLIC_KEY_BYTES_LENGTH + 1).join('0');

// Cole
const COLE_DUMMY_SIGNATURE = new Array(SIGNATURE_LENGTH + 1).join('0');
const COLE_DUMMY_PUBKEY = new Array(PUBLIC_KEY_BYTES_LENGTH + 1).join('0');

const CHAIN_CODE_DUMMY = new Array(CHAIN_CODE_LENGTH + 1).join('0');
const ATTRIBUTES_DUMMY = new Array(ATTRIBUTES_LENGTH + 1).join('0');

export interface BccService {
  /**
   * Derives a Sophie bech32 Enterprise address for the given public key
   *
   * @param networkId bcc network
  ...
  /**
   * Returns the era address type (either Sophie or Cole) based on an encoded string
   *
...
const signatureProcessor: { [eraType: string]: Signatures } = {
  [EraAddressType.Sophie]: {
    signature: SOPHIE_DUMMY_SIGNATURE,
    publicKey: SOPHIE_DUMMY_PUBKEY
  },
  [EraAddressType.Cole]: {
    signature: COLE_DUMMY_SIGNATURE,
    publicKey: COLE_DUMMY_PUBKEY,
...
const getWitnessesForTransaction = (logger: Logger, signatures: Signatures[]): BccWasm.TransactionWitnessSet => {
  try {
    ...
    signatures.forEach(signature => {
      ...
      if (signature.chain_code && signature.attributes) {
        ...
        bootstrapWitnesses.add(BccWasm.BootstrapWitness.new(vkey, ed25519Signature, chainCode, attributes));
      } else {
        vkeyWitnesses.add(BccWasm.Vkeywitness.new(vkey, ed25519Signature));
      }
    });
...
const configure = (depositParameters: DepositParameters): BccService => ({
  generateAddress(logger, network, publicKeyString, stakingCredentialString, type = AddressType.ENTERPRISE) {
    ...
    const pub = BccWasm.PublicKey.from_bytes(publicKeyBuffer);
  ...
  buildTransaction(logger, unsignedTransaction, signatures, metadata) {
    ...
    try {
      ...
      const transactionBody = BccWasm.TransactionBody.from_bytes(Buffer.from(unsignedTransaction, 'hex'));
      ...
      if (metadata) {
        ...
        auxiliaryData = BccWasm.AuxiliaryData.from_bytes(Buffer.from(metadata, 'hex'));
      }
      ...
      /* eslint-disable no-console */
      return hexFormatter(
        Buffer.from(BccWasm.Transaction.new(transactionBody, witnesses, auxiliaryData).to_bytes())
      );
    } catch (error) {
  ...
  createUnsignedTransaction(logger, network, operations, ttl): UnsignedTransaction {
    ...
    const transactionBody = BccWasm.TransactionBody.new(
      transactionInputs,
      transactionOutputs,
    ...
    if (voteRegistrationMetadata) {
      ...
      const metadataHash = BccWasm.hash_auxiliary_data(voteRegistrationMetadata);
    ...
    const bodyHash = BccWasm.hash_transaction(transactionBody).to_bytes();
  ...
  parseSignedTransaction(logger, networkId, transaction, extraData) {
    try {
      ...
      const parsed = BccWasm.Transaction.from_bytes(transactionBuffer);
  ...
  parseUnsignedTransaction(logger, networkId, transaction, extraData) {
    try {
      ...
      const transactionBuffer = Buffer.from(transaction, 'hex');
      const parsed = BccWasm.TransactionBody.from_bytes(transactionBuffer);
