...
import { BccService } from '../services/bcc-services';
  ...
  constructionDerive: async request =>
    withNetworkValidation(
      ...
      async () => {
        ...
        const address = bccService.generateAddress(
          logger,
          networkIdentifier,
          publicKey.hex_bytes,
          // eslint-disable-next-line camelcase
  ...
  constructionHash: async request =>
    withNetworkValidation(
      ...
      async () => {
        ...
        const transactionHash = bccService.getHashOfSignedTransaction(logger, signedTransaction);
  ...
  constructionPreprocess: async request =>
    withNetworkValidation(
      ...
      async () => {
        ...
        const transactionSize = bccService.calculateTxSize(
          request.log,
          networkIdentifier,
  ...
  constructionMetadata: async request =>
    withNetworkValidation(
      ...
      async () => {
        ...
        const updatedTxSize = bccService.updateTxSize(txSize, 0, ttl);
        ...
        const suggestedFee: BigInt = bccService.calculateTxMinimumFee(updatedTxSize, linearFeeParameters);
  ...
  constructionPayloads: async request =>
    withNetworkValidation(
      ...
      async () => {
        ...
        const unsignedTransaction = bccService.createUnsignedTransaction(
          logger,
          networkIdentifier,
          operations,
          parseInt(ttl)
  ...
  constructionCombine: async request =>
    withNetworkValidation(
      ...
      async () => {
        ...
        const signedTransaction = bccService.buildTransaction(
          logger,
          transaction,
      ...
      async () => {
        ...
        if (signed) {
          return {
            ...
            ...bccService.parseSignedTransaction(logger, networkIdentifier, transaction, extraData)
          };
        }
        return {
          // eslint-disable-next-line camelcase
          network_identifier: request.body.network_identifier,
          ...bccService.parseUnsignedTransaction(logger, networkIdentifier, transaction, extraData)
        };
      },
      request.log,
      networkService
    ...
    withNetworkValidation(
      ...
      async () => {
        try {
          ...
          await bccCli.submitTransaction(
            logger,
            signedTransaction,
