...
import { Pool } from 'pg';
import { BCC } from '../../../src/server/utils/constants';
import {
...
describe('/account/balance endpoint', () => {
  ...
  test('should return all utxos until last block if no block number is specified', async () => {
    ...
    expect(response.json()).toEqual({
      balances: [{ currency: { decimals: 6, symbol: 'BCC' }, value: '21063' }],
  ...
  test('should only consider balance up to block number if specified', async () => {
    const response = await server.inject({
      method: 'post',
      url: ACCOUNT_BALANCE_ENDPOINT,
      payload: generatePayload(BCC, 'mainnet', 'Ae2tdPwUPEZGvXJ3ebp4LDgBhbxekAH2oKZgfahKq896fehv8oCJxmGJgLt', 20)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json()).toEqual({
  ...
  test('should return empty if address doesnt exist', async () => {
    const response = await server.inject({
      method: 'post',
      url: ACCOUNT_BALANCE_ENDPOINT,
      payload: generatePayload(BCC, 'mainnet', 'fakeAddress', 44)
    });
  ...
  test('should return payment balance and list of ma balances', async () => {
    ...
    expect(response.json()).toEqual({
      ...
      balances: [
        { value: '24800000', currency: { symbol: 'BCC', decimals: 6 } },
        {
          value: '1000000',
          currency: {
            symbol: '64697361736d',
  ...
  test(
    ...
    async () => {
      ...
      expect(responseAtBlock5406841.json()).toEqual({
        ...
        balances: [
          { value: '110370442', currency: { decimals: 6, symbol: 'BCC' } },
          {
      ...
      expect(responseAtBlock5406842.json()).toEqual({
        ...
        balances: [
          { value: '107181709', currency: { symbol: 'BCC', decimals: 6 } },
          {
            value: '35000000000000',
            currency: {
              decimals: 0,
  ...
  test('should return balances for ma with empty name', async () => {
    ...
    const balanceWithEmptyToken = [
      { value: '106236428', currency: { symbol: 'BCC', decimals: 6 } },
      {
        value: '97614924',
        currency: {
          symbol: '4552474f',
