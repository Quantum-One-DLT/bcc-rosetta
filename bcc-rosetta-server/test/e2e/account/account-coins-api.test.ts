...
import { Pool } from 'pg';
import { BCC, ASSET_NAME_LENGTH, POLICY_ID_LENGTH } from '../../../src/server/utils/constants';
import {
...
describe('/account/coins endpoint', () => {
      ...
      coins: [
        {
          ...
          amount: { value: '10509579714', currency: { decimals: 6, symbol: 'BCC' } }
        },
        {
          coin_identifier: {
            identifier: 'bcc57134d1bd588b00f40142f0fdc17db5f35047e3196cdf26aa7319524c0014:1'
          },
          amount: { value: '999800000', currency: { decimals: 6, symbol: 'BCC' } }
        }
      ]
    });
  });
  ...
  test('should return empty if address doesnt exist', async () => {
    const response = await server.inject({
      method: 'post',
      url: ACCOUNT_COINS_ENDPOINT,
      payload: generatePayload(BCC, 'mainnet', 'fakeAddress')
    });
  ...
  test('should no return coins for stake accounts', async () => {
    const response = await server.inject({
      method: 'post',
      url: ACCOUNT_COINS_ENDPOINT,
      payload: generatePayload(BCC, 'mainnet', 'stake1uyqq2a22arunrft3k9ehqc7yjpxtxjmvgndae80xw89mwyge9skyp')
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json()).toEqual({
      block_identifier: latestBlockIdentifier,
  ...
  test('should return all coins when BCC is specified as currency', async () => {
    const response = await server.inject({
      method: 'post',
      url: ACCOUNT_COINS_ENDPOINT,
      payload: generatePayload(
  ...
  test('should fail when querying for a currency with non hex string symbol', async () => {
    ...
    const response = await server.inject({
      method: 'post',
      url: ACCOUNT_COINS_ENDPOINT,
      payload: generatePayload(BCC, 'mainnet', 'addr_test1vre2sc6w0zftnhselly9fd6kqqnmfmklful9zcmdh92mewszqs66y', [
        {
          decimals: 0,
