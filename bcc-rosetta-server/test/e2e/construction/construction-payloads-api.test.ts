...
import {
  ...
  CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_INPUT_RESPONSE,
...
describe(CONSTRUCTION_PAYLOADS_ENDPOINT, () => {
  ...
  test('Should return a valid unsigned transaction hash whenever a input with Cole Address is send', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PAYLOADS_ENDPOINT,
      payload: CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_INPUT
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json()).toEqual({
      unsigned_transaction: CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_INPUT_RESPONSE,
      payloads: [
        {
          account_identifier: {
