...
describe(CONSTRUCTION_PREPROCESS_ENDPOINT, () => {
  ...
  test('Should return a valid TTL when the parameters are valid', async () => {
    const response = await server.inject({
      ...
      // eslint-disable-next-line no-magic-numbers
      payload: generateProcessPayload({ blockchain: 'bcc', network: 'mainnet', relativeTtl: 100 })
    });
  ...
  test('Should return a valid TTL when the operations include an output with a Cole address', async () => {
    const { operations } = CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_INPUT;
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PREPROCESS_ENDPOINT,
      // eslint-disable-next-line no-magic-numbers
      payload: generateProcessPayload({ blockchain: 'bcc', network: 'mainnet', operations, relativeTtl: 100 })
    });

    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json()).toEqual({
      options: { relative_ttl: 100, transaction_size: sizeInBytes(SIGNED_TX_WITH_COLE_INPUT) }
    });
  });

  test('Should return a valid TTL when the operations include an output with a Cole address', async () => {
    const { operations } = CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_OUTPUT;
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PREPROCESS_ENDPOINT,
      // eslint-disable-next-line no-magic-numbers
    ...
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json()).toEqual({
      options: { relative_ttl: 100, transaction_size: sizeInBytes(SIGNED_TX_WITH_COLE_OUTPUT) }
    });
  });
  ...
  test('Should return a TTL when using default relateive ttl', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PREPROCESS_ENDPOINT,
      payload: generateProcessPayload({ blockchain: 'bcc', network: 'mainnet' })
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
