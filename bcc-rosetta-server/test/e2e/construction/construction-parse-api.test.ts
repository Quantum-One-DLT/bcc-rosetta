...
import {
  ...
  CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_OUTPUT,
  ...
  CONSTRUCTION_SIGNED_TX_WITH_COLE_ADDRESS_AND_EXTRA_DATA,
  ...
  CONSTRUCTION_SIGNED_TX_INPUT_WITH_COLE_ADDRESS_AND_EXTRA_DATA,
  CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_INPUT,
...
describe(CONSTRUCTION_PARSE_ENDPOINT, () => {
  ...
  test('Should return 1 input, 2 outputs and signers if a valid signed transaction is set', async () => {
    const response = await server.inject({
      ...
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_SIGNED_TRANSACTION_WITH_EXTRA_DATA)
    });
  ...
  test('Should return 1 input with cole address and signers if a valid signed transaction is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
    ...
    expect(response.json().operations).toEqual(
      constructionParseOperations(CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_INPUT)
    );
    expect(response.json().account_identifier_signers).toEqual([
      { address: CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_INPUT.operations[0].account?.address }
    ]);
  });

  test('Should return valid data if a valid signed transaction with a Cole address is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload(
    ...
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
      constructionParseOperations(CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_OUTPUT)
    );
    expect([response.json().account_identifier_signers[0].address]).toEqual([
      CONSTRUCTION_PAYLOADS_REQUEST_WITH_COLE_OUTPUT.operations[0].account?.address
    ]);
  });
  ...
  test('Should return 1 input, 2 outputs, 1 stake key registration and signers with payment and stake addresses', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_SIGNED_TX_WITH_REGISTRATION_AND_EXTRA_DATA)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
  ...
  test('Should return 1 input, 2 outputs and empty signers if a valid unsigned transaction is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', false, CONSTRUCTION_UNSIGNED_TRANSACTION_WITH_EXTRA_DATA)
    });
  ...
  test('Should return 1 input, 2 outputs, 1 stake key registration and empty signers if a valid unsigned transaction is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', false, CONSTRUCTION_PAYLOADS_STAKE_REGISTRATION_RESPONSE)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
      constructionParseOperations(CONSTRUCTION_PAYLOADS_WITH_STAKE_KEY_REGISTRATION)
  ...
  test('Should return 1 input, 2 outputs, 1 stake key deregistration and empty signers if a valid unsigned transaction is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', false, CONSTRUCTION_PAYLOADS_STAKE_DEREGISTRATION_RESPONSE)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
  ...
  test('Should return 1 input, 2 outputs, 1 stake delegation and empty signers if a valid unsigned transaction is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', false, CONSTRUCTION_PAYLOADS_STAKE_DELEGATION_RESPONSE)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
  ...
  test('Should return 1 input, 2 outputs, 1 withdrawal and empty signers if a valid unsigned transaction is set', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', false, CONSTRUCTION_PAYLOADS_WITHDRAWAL_RESPONSE)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
  ...
  test('Should throw an error when invalid signed transaction bytes are provided', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_INVALID_TRANSACTION)
    });
  ...
  test('Should throw an error when invalid unsigned transaction bytes are provided', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', false, CONSTRUCTION_INVALID_TRANSACTION)
    });
    expect(response.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
    expect(response.json()).toEqual({
      code: 4012,
  ...
  test('Should throw an error when valid unsigned transaction bytes but signed flag is true are provided', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_UNSIGNED_TRANSACTION_WITH_EXTRA_DATA)
    });
    expect(response.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
    expect(response.json()).toEqual({
      code: 4011,
  ...
  test('Should throw an error when valid signed transaction bytes but signed flag is false are provided', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', false, CONSTRUCTION_SIGNED_TRANSACTION_WITH_EXTRA_DATA)
    });
    expect(response.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
    expect(response.json()).toEqual({
      code: 4012,
  ...
  test('Should return 1 input and 2 ouput, first input and ouput with MultiAssets', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_SIGNED_TRANSACTION_WITH_MA)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
  ...
  test('Should correctly parse operations with two MultiAssets', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_SIGNED_TRANSACTION_WITH_MULTIPLE_MA)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
  ...
  test('Should correctly parse operations with several MultiAssets ', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_SIGNED_TRANSACTION_WITH_SEVERAL_MA)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
  ...
  test('Should correctly parse operations with MultiAssets without name ', async () => {
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_PARSE_ENDPOINT,
      payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_SIGNED_TX_WITH_MA_WITHOUT_NAME)
    });
    expect(response.statusCode).toEqual(StatusCodes.OK);
    expect(response.json().operations).toEqual(
  ...
  describe('Pool retirement requests', () => {
    ...
    test('Should return 1 input, 2 outputs, 1 pool retirement and empty signers if a valid unsigned transaction is set', async () => {
      const response = await server.inject({
        method: 'post',
        url: CONSTRUCTION_PARSE_ENDPOINT,
        payload: generateParsePayload('bcc', 'mainnet', true, CONSTRUCTION_SIGNED_TX_WITH_POOL_RETIREMENT)
      });
      expect(response.statusCode).toEqual(StatusCodes.OK);
      expect(response.json().operations).toEqual(
