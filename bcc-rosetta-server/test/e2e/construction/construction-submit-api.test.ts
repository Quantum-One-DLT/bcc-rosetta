...
import { bccCliMock, setupOfflineDatabase, setupServer } from '../utils/test-utils';
...
const ERROR_WHEN_CALLING_BCC_CLI = 'Error when calling bcc-cli';
...
describe(CONSTRUCTION_SUBMIT_ENDPOINT, () => {
  ...
  it('Should return the transaction identifier is request is valid', async () => {
    const mock = bccCliMock.submitTransaction as jest.Mock;
    mock.mockClear();
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_SUBMIT_ENDPOINT,
  ...
  it('Should return an error if there is a problem when sending the transaction', async () => {
    ...
    mock.mockImplementation(() => {
      throw new Error(ERROR_WHEN_CALLING_BCC_CLI);
    });
    const response = await server.inject({
      method: 'post',
      url: CONSTRUCTION_SUBMIT_ENDPOINT,
    ...
    expect(response.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
    expect((bccCliMock.submitTransaction as jest.Mock).mock.calls.length).toBe(1);
    expect(response.json()).toEqual({
      code: 5006,
      details: {
        message: ERROR_WHEN_CALLING_BCC_CLI
      },
