...
import { Pool } from 'pg';
import { BccCli } from '../../../src/server/utils/bcc/cli/bccnode-cli';
import { BccNode } from '../../../src/server/utils/bcc/cli/bcc-node';
...
export const bccCliMock: BccCli = {
  submitTransaction: jest.fn()
};

export const bccNodeMock: BccNode = {
  getBccNodeVersion: async (): Promise<string> =>
    'bcc-node 1.18.0 - linux-x86_64 - ghc-8.6\ngit rev 36ad7b90bfbde8afd41b68ed9b928df3fcab0dbc'
};
...
export const setupServer = (database: Pool): FastifyInstance => {
  ...
  return buildServer(services, bccCliMock, bccNodeMock, process.env.LOGGER_LEVEL, {
...
export const testInvalidNetworkParameters = (
  ...
  test('Should throw invalid network error when the network specified is invalid', async () => {
    const response = await server().inject({
      method: 'post',
      url: endpoint,
      payload: generateRequest('bcc', 'testnet')
    });
    expect(response.statusCode).toEqual(StatusCodes.INTERNAL_SERVER_ERROR);
    expect(response.json()).toEqual({
      code: 4002,
