...
A Node.js web service querying the postgres database produced by `bcc-db-sync`,
and interfacing with `bcc-node` via `bcc-cli`.
...
DB_CONNECTION_STRING="postgresql://postgres:mysecretpassword@127.0.0.1:5432/bcc-test"
# bcc-node topology absolute file path
TOPOLOGY_FILE_PATH="/etc/node/topology-test.json"
# Absolute path to the bcc-cli binary https://github.com/The-Blockchain-Company/bcc-node/tree/master/bcc-cli
BCC_CLI_PATH="/etc/node/bcc-cli"
# Absolute path to the bcc-node binary https://github.com/The-Blockchain-Company/bcc-node/tree/master/bcc-node
BCC_NODE_PATH="/etc/node/bcc-node"
# bcc-node genesis absolute file path
GENESIS_SOPHIE_PATH="/etc/node/testnet-genesis.json"
# bcc-node socket
BCC_NODE_SOCKET_PATH=/tmp/node.socket
...
**⚠ IMPORTANT NOTE: If `bcc-db-sync` is updated and it introduces a new schema, tests will still work as they were created with previous one. A manual update following the steps described below is required.**
...
1. Run a bcc node (it could be Bcc Rosetta)
2. When the node has reached the block you wish to export, execute `pg_dump $DB > network.bak`. For example, if you are using default Bcc Rosetta db running a mainnet node, you can run `pg_dump -U postgres cexplorer > mainnet.bak`.
3. Generate a `tar` file for `mainnet.bak` and place it in `/bcc-rosetta-server/test/e2e/jest-setup/mainnet-db-snapshot.tar`.
...
6. Generate a `tar` file for `network-fixture-data.sql` and place it in `/bcc-rosetta-server/test/e2e/jest-setup/network-fixture-data.tar`, for example `launchpad-fixture-data.tar`
...
- Introduce a way to check current schema in order to throw an error if `bcc-db-sync` is updated but it doesn't match the snapshot.
