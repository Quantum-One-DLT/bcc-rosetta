...
``` console
./bin/rosetta-cli check:data --configuration-file ./configuration/data/cole_sample.json
./bin/rosetta-cli check:data --configuration-file ./configuration/data/sophie_sample.json
./bin/rosetta-cli check:data --configuration-file ./configuration/data/sophie_transition.json
```
...
3. All the BCC (if any left) are sent to an address 

_Note: Running this example is complex as it requires a sender that owns tokens (for example by minting) and can send them to the rosetta-cli generated address. It's strongly recommended to use [this helper scripts](https://github.com/james-tbco/scripts) created by James that will make your life way much easier._

``` bash
# TOKENS_TO_RECEIVE_AMOUNT is the amount of tokens (non-bcc) that will be received
...
# RECIPIENT address that will receive the remaining BCC
...
``` bash
# TOKENS_TO_RECEIVE_AMOUNT_1 is the amount of tokens (non-bcc) that will be received
# TOKENS_TO_RECEIVE_AMOUNT_2 is the amount of tokens (non-bcc) of the second token that will be received
# MA_RECIPIENT_1 The address that will receive TOKENS_TO_RECEIVE_AMOUNT - 1
# MA_RECIPIENT_2 The address that will receive 1 token
...
# COST String that represents pool cost per epoch in entropic
# PLEDGE String that represents amount to pledge in entropic 
...
DNS_NAME="\"relays-new.bcc-testnet.tbcodev.io\""
COST="\"340000000\""
PLEDGE="\"799450000000\""
NUMERATOR="1"
DENOMINATOR="1"
