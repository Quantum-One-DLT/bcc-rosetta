...
if [ "$MODE" == "offline" ]; then
  echo 'Bcc Rosetta: Offline Mode';
  exec gosu postgres pm2-runtime start ecosystem.config.js --env production --only 'bcc-rosetta-server'
  ...
  echo "Bcc Rosetta: Invalid MODE: ${MODE}. If set, must be offline or online";
  exit 1
fi
